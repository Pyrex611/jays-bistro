  ...
  const MenuCard = ({ item, index }) => (
    ...
    >
      ...
      <div className="p-6 flex-grow flex flex-col relative">
        ...
        {/* START Mobile/Desktop Layout Switch */}
        <div className="flex flex-col sm:flex-row justify-between sm:items-center pt-4 border-t border-zinc-700/50 mt-auto gap-3">
          <span className="text-2xl font-black text-white tracking-tight">
            {formatPrice(item.price)}
          </span>
          <button 
            onClick={() => addToCart(item)}
            // Full width on mobile, rounded corners, height adjusted
            className="w-full sm:w-10 h-10 rounded-xl sm:rounded-full bg-orange-500 text-white flex items-center justify-center hover:bg-white hover:text-orange-600 transition-all duration-300 shadow-lg shadow-orange-500/20"
          >
            <span className="sm:hidden font-bold text-sm uppercase tracking-widest">Add to Order</span>
            <Plus size={20} className="sm:block hidden" />
          </button>
        </div>
        {/* END Mobile/Desktop Layout Switch */}
      </div>
    </div>
  );
  ...
  const ChatBot = () => {
    ...
    return (
      <>
        ...
        <div className={`fixed bottom-24 right-8 z-50 w-[90vw] max-w-sm bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-700 transition-all duration-300 origin-bottom-right ${isChatOpen ? 'scale-100 opacity-100' : 'scale-90 opacity-0 pointer-events-none'}`}>
